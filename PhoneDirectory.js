...
function phone(strng, num) {
	let contact = dr.split('\n').filter(i => i.includes(num));

	if (contact.length == 1) {
		contact = contact.join('');
	} else if (contact.length > 1) return 'Error => Too many people: ' + num;
	else return 'Error => Not found: ' + num;

	const name = contact.slice(contact.indexOf('<') + 1, contact.indexOf('>'));

	let adress =
		contact.slice(0, contact.indexOf('<')) +
		contact.slice(contact.indexOf('>') + 1, contact.length);

	adress = (
		adress.slice(0, adress.indexOf('+')) +
		adress.slice(adress.indexOf('+') + num.length + 1, adress.length)
	)
		.replace('_', ' ')
		.replace('$', '')
		.trim();
	...
	['*', ':', ';', '/'].forEach(item => {
		console.log(adress.indexOf(item) + ' ' + item);
		if (adress.includes(item)) adress = adress.replace(item, '');
	});

	adress = adress.replace('  ', ' ');
	return 'Phone => ' + num + ', Name => ' + name + ', Address => ' + adress.trim();
}
